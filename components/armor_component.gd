...
@export var payload_damage: float = 0.0 # Урон от содержимого (для снарядов)
...
func _on_armor_collision(other_area: Area2D):
	...
	if check_penetration(other_armor, hit_angle, impact_velocity):
		...
		other_armor.total_damage = handle_penetration(other_armor, hit_angle, impact_velocity)
		other_armor.apply_damage_to_health(self, thickness)
	else:
		# Рикошет
...
func check_penetration(other_armor: ArmorComponent, hit_angle: float, impact_velocity: float) -> bool:
	...
	var penetration_chance = (impact_velocity / min_impact_speed) * (1.0 / effective_thickness)
	
	# Чем больше угол - тем меньше вероятность пробития
	var angle_penalty = 1.0 - (hit_angle / 90.0) * 0.5
	penetration_chance *= angle_penalty
	print("Столкновение {0} с {1}, пробитие {2}".format([get_parent().name, other_armor.get_parent().name, randf() < penetration_chance]))
	return randf() < penetration_chance

func handle_penetration(penetrator: ArmorComponent, hit_angle: float, impact_velocity: float)->int:
	...
	print("Урон {0}, нанесен {1}".format([total_damage, penetrator.get_parent().name]))
